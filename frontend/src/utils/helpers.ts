...
export const calculateDefaultDeductions = (
  ...
  const assignments: Array<{ child_id: number; amount: number; shortage: number }> = []

  for (const child of children) {
    const targetAmount = average
    if (child.star_count >= targetAmount) {
      assignments.push({ child_id: child.id, amount: targetAmount, shortage: 0 })
      remaining -= targetAmount
    } else {
      // Use maximum available
      assignments.push({ child_id: child.id, amount: child.star_count, shortage: targetAmount - child.star_count })
